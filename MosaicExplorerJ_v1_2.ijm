...
while(isOpen(BoardID))
{
	...
	if(isKeyDown("alt"))
	{
		...
		if(ShowExtra)
		{
			...
			{
				Dialog.addMessage("To register CAM2:");
				Dialog.addMessage("1) Tick Register CAM2");
				Dialog.addMessage("2) Mark 2 distant matching points in both camera views");
				Dialog.addMessage("3) Tick register Tilt & Scaling");
				Dialog.addMessage("4) Mark 1 central matching point in both camera views");
				Dialog.addMessage("5) Tick register XY Position");
			}
			}
