...
while(isOpen(BoardID))
{
	...
	{
		if(NudgeMode==false)showStatus("Panel (Alt)  Nudge Z (Shift)  Z Step:"+d2s(Steps,0)+" (Space)  Register (Line+Alt)");
		else showStatus("Refresh (Alt)  Nudge ZCor (Shift)  Z Step:"+d2s(Steps,0)+" (Space)");
	}
	wait(50);
	...
	if(isKeyDown("alt"))
	{
		...
		if(ShowExtra)
		{
			...
			if(!isOpen("CAM1"))
			{
				if((ColorMode==true)||(CamCur==2))Dialog.addMessage("Disable ColorMode + set CAM1 to start guided CAM2 registration");
			}
			else 
			{
				Dialog.addMessage("To register CAM2, first mark 2 matching points in both images");
				Dialog.addMessage("and enter this menu again to register Tilt & Scaling");
				Dialog.addMessage("Then Mark 1 matching point in both images");
				Dialog.addMessage("and enter this menu again to register XY Position");
			}
			if((CamCur==1)&&(!isOpen("CAM1"))&&(ColorMode==false))Dialog.addCheckbox("Overlay CAM1", OverlayCAM1);
			if((CamCur==1)&&(!isOpen("CAM1"))&&(ColorMode==false))Dialog.addCheckbox("Register CAM2", false);
			if(CamCur==2)
			{	
				if(selectionType()==10)
				{
			...
			if((CamCur==1)&&(!isOpen("CAM1"))&&(ColorMode==false))OverlayCAM1 = Dialog.getCheckbox;
			else OverlayCAM1 = false;
			if(((CamCur==1)&&(!isOpen("CAM1"))&&(ColorMode==false))||((CamCur==2)&&(isOpen("CAM1"))&&(ColorMode==false)))RegCAM = Dialog.getCheckbox;
			else RegCAM = false;
			if(DualSide==1)
			{
